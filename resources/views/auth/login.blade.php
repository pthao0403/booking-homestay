...
<div class="auth-container" style="max-width: 450px; margin: 4rem auto;">
    <div class="auth-card" style="background: #f8fafc; padding: 2.5rem; border-radius: 16px; box-shadow: 0 10px 25px rgba(0,0,0,0.05); border: 1px solid #e2e8f0;">
        ...
        <form action="{{ route('login') }}" method="POST" class="auth-form bg-white p-4 rounded-3 border">
            ...
            @error('captcha')
                <div class="text-danger text-center mb-3" style="font-size: 0.9rem;">{{ $message }}</div>
            @enderror
        ...
        <p class="auth-link text-center mt-4 mb-0 text-muted" style="font-size: 0.9rem;">
            Chưa có tài khoản?
            <a href="{{ route('register') }}" class="text-primary fw-bold text-decoration-none">Đăng ký tại đây</a>
        </p>
    </div>
</div>
@endsection
